...
# create list of strings of collaborator (client) values, for use later
collaborator_list = [str(i) for i in range(NUM_COLLABORATORS)]
fx.init(col_names=collaborator_list)

# wrapper around keras model, passed args directly from federated model
# adam optimizer for large data, softmax for classification
...
# separate into training, validation, testing sets
...
# separate input values from output targets
# convert to numpy for compatibility with keras model
